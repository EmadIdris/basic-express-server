...
describe('The web server', () => {
  test('gives a 404 on a bad route', async () => {
    const response = await mockRequest.put('/tacos');
    expect(response.status).toBe(404);
  });
  test('gives a 404 on a bad CRUD method', async () => {
    const response = await mockRequest.put('/person');
    expect(response.status).toBe(404);
  });
  test('gives a 500 if no name was given', async () => {
    const response = await mockRequest.get('/person/');
    expect(response.status).toBe(500);
  });
  test('gives a 200 is a name was given', async () => {
    const response = await mockRequest.get('/person/?name=test');
    expect(response.status).toBe(200);
  });
  test('returns the correct object for the given name', async () => {
    const response = await mockRequest.get('/person/?name=test');
